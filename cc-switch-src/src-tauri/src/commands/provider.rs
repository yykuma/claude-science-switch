...
const SCIENCE_DISABLED_ACCOUNT_USAGE_MESSAGE: &str =
    "Managed OAuth and official-account usage queries are disabled in Claude Science Switch.";
...
async fn query_provider_usage_inner(
    state: &AppState,
    _copilot_state: &CopilotAuthState,
    app_type: AppType,
    provider_id: &str,
) -> Result<crate::provider::UsageResult, String> {
    ...
    if provider
        .map(|p| p.uses_science_disabled_account_path())
        .unwrap_or(false)
    {
        return Ok(disabled_science_account_usage_result());
    }
    ...
    if template_type == TEMPLATE_TYPE_GITHUB_COPILOT {
        return Ok(crate::provider::UsageResult {
            ...
            data: None,
            error: Some(
                "GitHub Copilot managed OAuth usage is disabled in Claude Science Switch."
                    .to_string(),
            ),
        });
    }
...
pub async fn testUsageScript(
    ...
    if providers
        .get(&providerId)
        .map(|provider| provider.uses_science_disabled_account_path())
        .unwrap_or(false)
    {
        return Ok(disabled_science_account_usage_result());
    }
...
fn disabled_science_account_usage_result() -> crate::provider::UsageResult {
    crate::provider::UsageResult {
        success: false,
        data: None,
        error: Some(SCIENCE_DISABLED_ACCOUNT_USAGE_MESSAGE.to_string()),
    }
}
