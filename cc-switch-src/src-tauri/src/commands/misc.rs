...
pub async fn check_for_updates(handle: AppHandle) -> Result<bool, String> {
    handle
        .opener()
        .open_url(
            "https://github.com/yykuma/claude-science-switch/releases/latest",
            None::<String>,
        )
...
pub async fn run_tool_lifecycle_action(
    ...
    if requested.contains(&"claude") {
        return Err(
            "Claude Science Switch does not install or update Claude Code automatically"
                .to_string(),
        );
    }
...
pub async fn open_provider_terminal(
    ...
    if matches!(app_type, AppType::Claude | AppType::ClaudeDesktop) {
        return Err(
            "Claude Science Switch refuses to launch the real claude CLI from provider profiles"
                .to_string(),
        );
    }
