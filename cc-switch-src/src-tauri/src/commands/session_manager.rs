...
pub async fn launch_session_terminal(
    ...
    if looks_like_real_claude_launch(&command) {
        return Err(
            "Claude Science Switch refuses to launch the real claude CLI from session history"
                .to_string(),
        );
    }
...
fn looks_like_real_claude_launch(command: &str) -> bool {
    let normalized = command.trim_start();
    normalized.starts_with("claude ")
        || normalized.contains(" claude --settings ")
        || normalized.contains("'claude --settings ")
        || normalized.contains("\"claude --settings ")
}
