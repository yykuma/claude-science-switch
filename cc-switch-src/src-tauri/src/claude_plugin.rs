...
fn claude_dir() -> Result<PathBuf, AppError> {
    ...
    crate::config::guard_science_managed_path(&dir, "Claude plugin config dir")?;
    Ok(dir)
}
...
pub fn ensure_claude_dir_exists() -> Result<PathBuf, AppError> {
    let dir = claude_dir()?;
    crate::config::guard_science_managed_path(&dir, "Claude plugin config dir create")?;
    if !dir.exists() {
...
pub fn write_claude_config() -> Result<bool, AppError> {
    ...
    let path = claude_config_path()?;
    crate::config::guard_science_managed_path(&path, "Claude plugin write")?;
...
pub fn clear_claude_config() -> Result<bool, AppError> {
    let path = claude_config_path()?;
    crate::config::guard_science_managed_path(&path, "Claude plugin clear")?;
    if !path.exists() {
        return Ok(false);
    }
