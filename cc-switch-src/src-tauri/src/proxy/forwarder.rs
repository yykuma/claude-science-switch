...
const MANAGED_AUTH_DISABLED_ERROR: &str =
    "Managed OAuth/official account providers are disabled in Claude Science Switch. Use an explicit API key + base URL provider instead.";
    ...
    async fn forward(
        ...
        if provider.uses_science_disabled_account_path() {
            return Err(ProxyError::ConfigError(
                MANAGED_AUTH_DISABLED_ERROR.to_string(),
            ));
        }
