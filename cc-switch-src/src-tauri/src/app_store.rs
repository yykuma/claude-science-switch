...
fn read_override_from_store(app: &tauri::AppHandle) -> Option<PathBuf> {
    ...
    match store.get(STORE_KEY_APP_CONFIG_DIR) {
        Some(Value::String(path_str)) => {
            ...
            if is_forbidden_app_config_override(&path) {
                log::warn!("Store 中配置的 app_config_dir 指向受保护路径，已拒绝: {path:?}");
                return None;
            }
    ...
    match path {
        Some(p) => {
            ...
            if !trimmed.is_empty() {
                ...
                if is_forbidden_app_config_override(&resolved) {
                    return Err(AppError::Config(format!(
                        "refusing app_config_dir override that points at a protected path: {}",
                        resolved.display()
                    )));
                }
...
fn is_forbidden_app_config_override(path: &std::path::Path) -> bool {
    let home = crate::config::get_home_dir();
    let forbidden = [
        home.join(".cc-switch"),
        home.join(crate::config::SCIENCE_CONFIG_DIR_NAME),
        home.join(".claude"),
        home.join(".claude.json"),
    ];
    forbidden
        .iter()
        .any(|root| crate::config::path_matches_or_is_within(path, root))
}
