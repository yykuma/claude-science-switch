...
fn is_supported_deeplink_url(url_str: &str) -> bool {
    url_str.starts_with("claude-science-switch://") || url_str.starts_with("ccswitch://")
}

/// 统一处理 Claude Science Switch 深链接 URL
///
/// - 解析 URL
...
pub fn run() {
    ...
    let builder = builder
        ...
        .setup(|app| {
            ...
            // Claude Science Switch disables the bundled updater until a
            // dedicated signed release feed exists for this fork.
        ...
        .invoke_handler(tauri::generate_handler![
            ...
            commands::get_science_proxy_process_status,
            commands::start_science_proxy_process,
            commands::stop_science_proxy_process,
            commands::run_science_proxy_preflight,
            commands::get_science_app_process_status,
            commands::launch_science_app_with_proxy,
            commands::stop_science_app_process,
            commands::open_science_proxy_config_folder,
            commands::open_science_app_profile_folder,
    ...
    app.run(|app_handle, event| {
        ...
        if let RunEvent::ExitRequested { api, code, .. } = &event {
            match classify_exit_request(*code) {
                ...
                //   - 托管 Claude Science/proxy：只做同步进程清理，不触碰窗口状态/DB，避免残留子进程
                ...
                ExitRequestAction::DeferToTauriRestart => {
                    ...
                    if let Err(e) = commands::stop_managed_science_app_process() {
                        log::warn!("重启前停止 Claude Science 隔离实例失败: {e}");
                    }
                    if let Err(e) = commands::stop_managed_science_proxy_process() {
                        log::warn!("重启前停止 Claude Science Switch 托管代理失败: {e}");
                    }
                    return;
                }
...
pub async fn cleanup_before_exit(app_handle: &tauri::AppHandle) {
    if let Err(e) = commands::stop_managed_science_app_process() {
        log::warn!("退出时停止 Claude Science 隔离实例失败: {e}");
    }

    if let Err(e) = commands::stop_managed_science_proxy_process() {
        log::warn!("退出时停止 Claude Science Switch 托管代理失败: {e}");
    }
