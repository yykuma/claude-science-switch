...
use crate::config::{
    delete_file, get_claude_settings_path, guard_science_managed_path, read_json_file,
    write_json_file,
};
    ...
    pub(crate) fn restore(&self) -> Result<(), AppError> {
        match self {
            LiveSnapshot::Claude { settings } => {
                ...
                guard_science_managed_path(&path, "Claude live restore")?;
...
pub(crate) fn write_live_snapshot(app_type: &AppType, provider: &Provider) -> Result<(), AppError> {
    match app_type {
        AppType::Claude => {
            let path = get_claude_settings_path();
            guard_science_managed_path(&path, "Claude live write")?;
...
pub fn read_live_settings(app_type: AppType) -> Result<Value, AppError> {
    match app_type {
        ...
        AppType::Claude => {
            let path = get_claude_settings_path();
            guard_science_managed_path(&path, "Claude live read")?;
            if !path.exists() {
                return Err(AppError::localized(
                    "claude.live.missing",
                    "Claude Code 配置文件不存在",
...
pub fn import_default_config(state: &AppState, app_type: AppType) -> Result<bool, AppError> {
    ...
    let settings_config = match app_type {
        ...
        AppType::Claude => {
            ...
            guard_science_managed_path(&settings_path, "Claude live import")?;
