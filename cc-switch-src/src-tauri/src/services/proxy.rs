...
impl ProxyService {
    ...
    pub async fn sync_claude_live_from_provider_while_proxy_active(
        ...
        if provider.uses_science_disabled_account_path() {
            return Err(MANAGED_AUTH_DISABLED_MESSAGE.to_string());
        }
    ...
    fn reject_managed_account_current_provider(&self, app_type: &AppType) -> Result<(), String> {
        if !matches!(app_type, AppType::Claude | AppType::ClaudeDesktop) {
            return Ok(());
        }

        if let Some(provider) = self.get_current_provider_for_app(app_type)? {
            if provider.uses_science_disabled_account_path() {
                return Err(format!(
                    "{MANAGED_AUTH_DISABLED_MESSAGE} 当前供应商：{}",
                    provider.name
                ));
            }
        }
    ...
    pub async fn start_with_takeover(&self) -> Result<ProxyServerInfo, String> {
        self.reject_managed_account_current_provider(&AppType::Claude)?;
    ...
    pub async fn set_takeover_for_app(&self, app_type: &str, enabled: bool) -> Result<(), String> {
        ...
        if enabled {
            self.reject_managed_account_current_provider(&app)?;
    ...
    async fn takeover_live_configs(&self) -> Result<(), String> {
        ...
        if let Ok(mut live_config) = self.read_claude_live() {
            ...
            if claude_provider.uses_science_disabled_account_path() {
                return Err(MANAGED_AUTH_DISABLED_MESSAGE.to_string());
            }
            Self::apply_claude_takeover_fields_for_provider(
                &mut live_config,
