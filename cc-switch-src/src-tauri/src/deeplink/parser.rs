...
//!
//! Parses Claude Science Switch deep link URLs into DeepLinkImportRequest structures.
...
/// Parse a Claude Science Switch URL into a DeepLinkImportRequest
///
/// Expected format:
/// claude-science-switch://v1/import?resource={type}&...
pub fn parse_deeplink_url(url_str: &str) -> Result<DeepLinkImportRequest, AppError> {
    ...
    if !matches!(scheme, "claude-science-switch" | "ccswitch") {
        return Err(AppError::InvalidInput(format!(
            "Invalid scheme: expected 'claude-science-switch' or 'ccswitch', got '{scheme}'"
        )));
    }
