...
fn build_provider_meta(request: &DeepLinkImportRequest) -> Result<Option<ProviderMeta>, AppError> {
    ...
    // Deep links are external input. Import script code, but never enable it
    // automatically; the user can review and enable it from the app.
    let enabled = request.usage_enabled.unwrap_or(false) && code.is_empty();
