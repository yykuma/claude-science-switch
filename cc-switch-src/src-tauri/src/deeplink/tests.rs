...
#[test]
fn test_parse_valid_science_switch_deeplink() {
    let url =
        "claude-science-switch://v1/import?resource=provider&app=claude&name=Science%20Provider";
    ...
    assert_eq!(request.resource, "provider");
    assert_eq!(request.app, Some("claude".to_string()));
    assert_eq!(request.name, Some("Science Provider".to_string()));
}
...
#[test]
fn test_deeplink_usage_script_code_imports_disabled() {
    use super::provider::build_provider_from_request;
    ...
    let request = DeepLinkImportRequest {
        ...
        resource: "provider".to_string(),
        app: Some("claude".to_string()),
        name: Some("Scripted Claude".to_string()),
        homepage: Some("https://example.com".to_string()),
        endpoint: Some("https://api.example.com/v1".to_string()),
        api_key: None,
        icon: None,
        ...
        enabled: None,
        usage_enabled: Some(true),
        usage_script: Some("Y29uc29sZS5sb2coMSk=".to_string()),
        usage_api_key: None,
        usage_base_url: None,
        usage_access_token: None,
        usage_user_id: None,
    ...
    let script = provider
        ...
        .as_ref()
        .and_then(|meta| meta.usage_script.as_ref())
        .expect("usage script should be imported");

    assert!(!script.enabled);
    assert_eq!(script.code, "console.log(1)");
}
