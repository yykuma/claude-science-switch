...
pub fn load_messages(provider_id: &str, source_path: &str) -> Result<Vec<SessionMessage>, String> {
    ...
    let (validated_source, _) =
        validate_source_path_under_roots(provider_id, Path::new(source_path), &roots)?;
    match provider_id {
...
fn delete_session_with_roots(
    ...
    let (validated_source, validated_root) =
        validate_source_path_under_roots(provider_id, source_path, roots)?;
...
fn validate_source_path_under_roots(
    provider_id: &str,
    source_path: &Path,
    roots: &[PathBuf],
...
mod tests {
    ...
    #[test]
    fn rejects_load_source_path_outside_provider_root() {
        let root = tempdir().expect("tempdir");
        let outside = tempdir().expect("tempdir");
        let source = outside.path().join("session.jsonl");
        std::fs::write(&source, "{}").expect("write source");

        let err = validate_source_path_under_roots("codex", &source, &[root.path().to_path_buf()])
            .expect_err("expected outside-root path to be rejected");
