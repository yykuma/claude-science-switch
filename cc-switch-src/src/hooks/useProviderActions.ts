...
export function useProviderActions(
  ...
  const switchProvider = useCallback(
    async (provider: Provider) => {
      const providerSettings = provider.settingsConfig as Record<string, any>;
      const providerEnv = providerSettings?.env as
        | Record<string, any>
        | undefined;
      const claudeBaseUrl = String(
      ...
      const isManagedAuthProvider =
        (activeApp === "claude" || activeApp === "claude-desktop") &&
        (provider.category === "official" ||
          provider.meta?.providerType === "github_copilot" ||
          provider.meta?.providerType === "codex_oauth" ||
          claudeBaseUrl.includes("githubcopilot.com") ||
          claudeBaseUrl.includes("chatgpt.com/backend-api/codex"));

      if (isManagedAuthProvider) {
        toast.error(
          t("providerForm.managedAuthDisabled", {
            defaultValue:
