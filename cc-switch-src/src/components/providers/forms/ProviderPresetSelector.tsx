...
function isScienceSafePreset(preset: AnyPreset): boolean {
  const providerType = (preset as { providerType?: string }).providerType;
  return (
...
export function isScienceSafePresetEntry(entry: PresetEntry): boolean {
  return isScienceSafePreset(entry.preset);
}
...
export function ProviderPresetSelector({
  ...
  const showUniversalProviderPresets = false;
  ...
  const sciencePresetEntries = useMemo(
    () => presetEntries.filter(isScienceSafePresetEntry),
    [presetEntries],
  );

  const visiblePresetEntries = useMemo(
    () =>
      getVisiblePresetEntries(sciencePresetEntries, {
        query: searchQuery,
        sortMode,
        t,
      }),
    [sciencePresetEntries, searchQuery, sortMode, t],
  );
